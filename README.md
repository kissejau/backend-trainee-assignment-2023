...
- REST API написано на языке `golang` с использованием библиотеки `julienschmidt/httprouter` (надстройка над std библиотекой net/http)
- БД - `postgresql`, ОРМ не использовалась, писались чистые запросы
- Приложение разворачивается в докер контейнере, используется `docker`, `docker-compose`
- Для поднятия таблиц БД были созданы миграции, с помощью инструмента `migrate`
- Реализовано логирование в консоль на запросы к серверу с помощью библиотеки `zerolog`

## Вопросы

- Доп. задание 1: на вход даются год-месяц, про идентификатор пользователя ни слова, поэтому реализовал лог всех пользователей, не фильтруя по конкретному, только по дате.
- Доп. задание 2: как я понял, реалиховывать удаление нет необходимости, поэтому сделана только возможность добавлять TTL и при получении сегментов пользователя добавлен флаг активности сегмента.

## Не сделано, но могло бы быть..

- swagger документация
- тесты
- ci/cd
- нормальный error handling
- доп. задание 3
...
- Тип содержимого ответа (code 202)
  - `text/csv`
